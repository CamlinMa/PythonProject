...
class Kmeans:
    def __init__(self, k, pointList,weight, kmeansThreshold, initialCentroids=None):
        self.pointList = []
        self.numPoints = len(pointList)
        self.k = k # 聚类数
        self.initPointList = [] # 初始质心点
        self.dim = len(pointList[0])
        self.kmeansThreshold = kmeansThreshold # 误差控制
        self.error = None
        self.errorList = []
        self.weight=weight # 特征权重
        i = 0
        ...
        if initialCentroids is not None:
            ...
            self.centroidList = self.selectSeeds(self.k) # 生成随机初始聚类质心
        self.mainFunction()
    ...
    def seeds(self, initList):
        ...
        for seed in initList:
            centroidList.append(Centroid(Point(seed,len(seed))))
        return centroidList

    def getDistance(self, point1, point2,weight):
        ''' 计算欧氏距离 对距离按维度加权  '''
        distance = 0
        for x in range(0, self.dim):
            distance +=(((float(point1.coordinates[x]) -float(point2.coordinates[x])) ** 2)*weight[x])
        return (distance) ** (0.5)
    ...
    def getCentroid(self, point):
        ...
        for centroid in self.centroidList:
            dist = self.getDistance(point, centroid.point,self.weight)
            if minDist == -1:
                minDist = dist
                closestCentroid = pos
            elif minDist > dist:
    ...
    def reCalculateCentroid(self):
        ...
        for centroid in self.centroidList:
            ...
            for point in centroid.pointList:
                for x in range(0, self.dim):
                    mean.coordinates[x] += float(point.coordinates[x])
            for x in range(0, self.dim):
                try:
    ...
    def calculateError(self, config):
        ...
        for centroid in self.centroidList:
            for point in centroid.pointList:
                error += self.getDistance(point, centroid.point,self.weight) ** 2
        return error
